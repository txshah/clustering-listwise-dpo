...
def score_question(
    ...
    if len(corrects) == 1:
        ...
        correct_scores = score_pairs(
            premises    = [ref_tail] * len(corrects),
            hypotheses  = [c[-_TAIL_CHARS:] for c in corrects],
            tokenizer   = tokenizer,
            model       = model,
            device      = device,
    ...
    wrong_scores = []
    if wrongs:
        wrong_scores = score_pairs(
            premises    = [ref_tail] * len(wrongs),
            hypotheses  = [w[-_TAIL_CHARS:] for w in wrongs],
            tokenizer   = tokenizer,
            model       = model,
            device      = device,
            batch_size  = batch_size,
